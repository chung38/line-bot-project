...
const loadInviter = async () => {
  try {
    ...
    groupInviter = new Map(Object.entries(JSON.parse(d)));
    console.log("✅ 載入邀請者設定");
  } catch {}
};

const saveInviter = async () => {
  const obj = Object.fromEntries(groupInviter);
    ...
    await Promise.all(req.body.events.map(async event => {
      ...
      if (event.type === "message" && txt === "!設定" && gid) {
        ...
        if (groupInviter.get(gid) === uid) {
          ...
          await client.replyMessage(event.replyToken, { type: "text", text: "只有邀請者可以設定語言選單。" });
        }
        return;
      }
      ...
      if (event.type === "message" && event.message.type === "text" && gid) {
        ...
        const profile = await client.getGroupMemberProfile(gid, uid).catch(() => null);
        const name = profile?.displayName || "未知用戶";

        let translations;
        if (isChinese(txt)) {
          const results = await Promise.all([...set].map(code => translateWithDeepSeek(txt, code)));
          translations = results.map(res => `【${name}】說：${res}`);
        } else {
          const res = await translateWithDeepSeek(txt, "zh-TW");
          translations = [`【${name}】說：${res}`];
        }

        await client.replyMessage(event.replyToken, translations.map(t => ({ type: "text", text: t })));
      }
    }));
    res.sendStatus(200);
  }
