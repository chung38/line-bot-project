...
async function translateWithDeepSeek(text, targetLang) {
  ...
  try {
    const response = await axios.post(
      ...
      { model: "deepseek-chat", messages: [{ role: "system", content: `專業翻譯成 ${targetLang}：` }, { role: "user", content: text }] },
...
async function sendLanguageMenu(groupId) {
  ...
  await lineClient.pushMessage(groupId, { type: "flex", altText: "翻譯設定", contents: { type: "bubble", header: { type: "box", layout: "vertical", contents: [{ type: "text", text: "🌍 翻譯設定" }] }, body: { type: "box", layout: "vertical", contents: [{ type: "text", text: "✔ 已選: " + Array.from(selected).map(l => languageNames[l]).join(", ") }, { type: "separator", margin: "md" }, ...buttons ] } } });
}
...
async function handleMessage(event) {
  ...
  if (!selectedLangs.size || selectedLangs.has("no-translate")) return lineClient.replyMessage(event.replyToken, { type: "text", text: "⚠️ 請先使用「!設定」選擇語言" });
}
