...
const translateBatch = async (text, codes, retry=0) => {
  ...
  const langList = codes.map(c=>`${names[c]}(${c})`).join("、");
  const sys = `你是一名翻譯員，請將以下句子同時翻譯成${langList}，僅回傳 JSON 格式，如 {"${codes[0]}":"...","${codes[1]}":"..."}。`;
  try {
    const res = await axios.post(
      "https://api.deepseek.com/v1/chat/completions",
      { model:"deepseek-chat", messages:[{role:"system",content:sys},{role:"user",content:text}] },
    ...
    const json = JSON.parse(res.data.choices[0].message.content.trim());
    ...
    console.warn("批次翻譯失敗，退回單語:", e.message);
    const out = {};
    for (const c of codes) {
      out[c] = await translateSingle(text, c);
    }
    return out;
  }
};
  ...
  async (req, res) => {
    await Promise.all(req.body.events.map(async event => {
      ...
      if (event.type==="message" && event.message.type==="text" && gid) {
        ...
        if (isChinese(txt)) {
          const codes = [...set];
          const batch = await translateBatch(txt,codes);
          const msgs = codes.map(c=>({ type:"text", text: batch[c] }));
...
const sendMenu = async (gid,retry=0) => {
  ...
  const msg={ type:"flex", altText:"語言設定選單", contents:{
    type:"bubble", body:{
  ...
  catch(e){ if(e.statusCode===429&&retry<3){ await new Promise(r=>setTimeout(r,(retry+1)*5000)); return sendMenu(gid,retry+1);} console.error("選單發送失敗:",e.message);}
};
