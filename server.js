...
// 批次多語翻譯
const translateBatch = async (text, codes) => {
  const names = { en:"英文", th:"泰文", vi:"越南文", id:"印尼文" };
  const promptLangs = codes.map(c => `${names[c]}(${c})`).join("、");
  const sys = `你是一名翻譯員，請將以下句子同時翻譯成${promptLangs}，僅回傳 JSON 格式，例如 {"${codes[0]}":"...","${codes[1]}":"..."}，不要其他文字。`;
  try {
    const res = await axios.post(
      "https://api.deepseek.com/v1/chat/completions",
      { model:"deepseek-chat", messages:[{role:"system",content:sys},{role:"user",content:text}] },
    ...
    for (const c of codes) {
      results[c] = await translateWithDeepSeek(text, c);
    }
    return results;
  }
};
